...
// 渲染单个表单项
const renderFormItem = ({ item, formLayout, getFieldDecorator }) => {
    if (Array.isArray(item)) {
        return (
            <Row>
                {item.map(childItem =>
                    renderFormItem({ item: childItem, formLayout, getFieldDecorator })
                )}
            </Row>
        );
    }
    ...
    if (formLayout === FORM_LAYOUT_MODEL.horizontal) {
        formItemLayout = itemLayout ? { ...itemLayout } : { ...defaultFormItemLayout };
    }
    return (
        <Fragment key={key}>
            {col ? (
                ...
                <FormItem label={label} {...formItemLayout}>
                    {getFieldDecorator(key, {
                        ...options,
                        rules: rules || [{ required, message: `${label}为空` }],
                    })(component || <Input style={{ ...wrapStyle }} placeholder={placeholder} />)}
                </FormItem>
            )}
        </Fragment>
    );
...
class AdvancedForm extends PureComponent {
    ...
    render() {
        ...
        return (
            <Form layout={formLayout || defaultFormLayout} onSubmit={this.handleCommonSubmit}>
                {items.map(item => renderFormItem({ item, formLayout, getFieldDecorator }))}
                {this.renderButtons()}
            </Form>
        );
    }
